...
export interface SocketComponentProps extends PropsWithChildren {}

const SocketContextComponent: React.FunctionComponent<SocketComponentProps> = (
  props
) => {
  ...
  const SendHandshake = () => {
    ...
    socket.emit("handshake", async (uid: string, users: string[]) => {
      ...
      SocketDispatch({ type: "update_users", payload: users });
      SocketDispatch({ type: "update_uuidv4", payload: uid });
    });
